...
const schema = z.object({
    dayOfWeek: z.string().min(1, {message: 'Please select a day of week'}),
    slotList: z.array(z.object({
...
export function AddEditAvailability({data, mode = 'add', handleCancel}) {
    ...
    const shiftsMaster = [
        {label: t('morning'), value: 'morning', disabled: false},
        {label: t('afternoon'), value: 'afternoon', disabled: false},
    ...
    const {handleSubmit, watch, getValues, formState: {errors, isSubmitting}, control} = useForm({
        mode: 'onChange',
        defaultValues: {
            dayOfWeek: shiftsData?.[0]?.dayOfWeek.toLowerCase() || '',
            slotList: shiftsData?.map(item => ({
    ...
    useEffect(() => {
        ...
        setAvailableShifts(prevShifts =>
            prevShifts.map(shift => ({
                ...shift,
                disabled: shiftValues.includes(shift.value)
            }))
        );
    }, [watch('slotList')]);
    ...
    const addShift = () => {
        ...
        setAvailableShifts((prev) =>
            prev.map((shift) => ({...shift, disabled: selectedShifts.includes(shift.value)}))
        );
    };
    ...
    const removeShift = (index) => {
        ...
        setAvailableShifts((prev) =>
            prev.map((shift) => (shift.value === removedSlotType ? {...shift, disabled: false} : shift))
        );
    ...
    const handleShiftChange = (field, value, index) => {
        ...
        setAvailableShifts(prev =>
            prev.map((shift, idx) => idx === index ? {...shift, disabled: true} : shift)
        );
    ...
    const saveUpdateTiming = async (payload) => {
        ...
        try {
            const response = mode === 'add'
                ? await http.post(apiConfig.doctors.saveDoctorAvailability, payload) :
                await http.put(apiConfig.doctors.updateDoctorAvailability, payload);
